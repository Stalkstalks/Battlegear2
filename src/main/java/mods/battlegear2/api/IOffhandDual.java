...
public interface IOffhandDual {
    ...
    /**
     * Perform any function when this item is held in the offhand (left) and the user right clicks an entity. This is
     * generally used to apply an effect on attacking the entity with the offhand item. If this is the case the
     * {@link PlayerEventChild.OffhandAttackEvent#cancelParent} field should be left at true, and the event should NOT
     * be cancelled, to prevent default right clicking events: (Eg PlayerInteractEvent, Item#onItemRightClick) being
     * called for the item in right hand. Leave {@link PlayerEventChild.OffhandAttackEvent#shouldAttack} field at true,
     * to perform the "vanilla" attack procedure and receive #onLeftClickEntity(ItemStack, EntityPlayer, Entity) in
     ...
     * happen prior to the activation of any functions of blocks Note: Called first on client-side, then on server side
