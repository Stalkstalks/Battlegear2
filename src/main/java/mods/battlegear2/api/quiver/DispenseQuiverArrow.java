...
/**
 * Created by GotoLink on 08/11/2014. Dispense arrows from {@link IArrowContainer2} Needs to be added to
 * {@link BlockDispenser#dispenseBehaviorRegistry} with the corresponding arrow container item
 */
public class DispenseQuiverArrow extends DispenseArrow {
    ...
    /**
     * The charge value used when instantiating the arrow from
     * {@link IArrowContainer2#getArrowType(ItemStack, World, EntityPlayer, float)}
     */
    ...
    protected EntityArrow getArrowEntity(World world, ItemStack itemStack) {
        if (itemStack.getItem() instanceof IArrowContainer2) {
            ...
            if (((IArrowContainer2) itemStack.getItem()).hasArrowFor(
                    itemStack,
                    bowStack,
                    fake,
                    ((IArrowContainer2) itemStack.getItem()).getSelectedSlot(itemStack))) {
                EntityArrow arrow = ((IArrowContainer2) itemStack.getItem())
                        .getArrowType(itemStack, world, fake, charge);
                if (arrow != null) {
    ...
    /**
     * Simulates a stack containing a bow compatible with the arrow container. Necessary to make a fake, since dispenser
     * doesn't keep the bow data it was crafted with.
     *
    ...
    /**
     * Simulates a player using a bow to fire the arrow from the container Necessary to build a fake, since redstone
     * doesn't identify source of change
     *
