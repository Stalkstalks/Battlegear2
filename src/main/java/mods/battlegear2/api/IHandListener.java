...
/**
 * Created by GotoLink on 29/09/2014. To be implemented by items wishing to receive click for the wielding hand without
 * having to register listener classes
 */
public interface IHandListener {
     ...
     *
     * @param event         A shallow copy of the interaction event that triggered Cancelling the event will not prevent
     *                      the server side call, but will prevent block activation. WHEN isOffhand is TRUE,
     *                      {@link PlayerInteractEvent#useItem} decide whether to use the offhandStack, while the
