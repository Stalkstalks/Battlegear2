...
public class CircleTool extends RectangleTool {
    ...
    protected void drawShape2(int minX, int minY, int maxX, int maxY, int[] pixels, int rgb) {
        ...
        for (float time = 0; time < Math.PI / 2; time += 0.05F) {
            plotPoint(
                    ...
                    (int) Math.round(y_mid + y_rad * Math.sin(time)),
                    pixels,
                    rgb);
            plotPoint(
                    (int) Math.round((x_mid - x_rad * Math.cos(time))),
    ...
    protected void drawShape(int minX, int minY, int maxX, int maxY, int[] pixels, int rgb) {
        ...
        for (int x = 1; x <= x_rad; x++) {
            ...
            drawLine(
                    (int) Math.ceil(x_mid + last_x),
                    (int) Math.ceil(x_mid + x),
                    (int) Math.ceil(y_mid + last_y),
                    (int) Math.ceil(y_mid + y),
                    pixels,
                    rgb);
            drawLine(
                    (int) Math.floor(x_mid - last_x),
                    (int) Math.floor(x_mid - x),
                    (int) Math.ceil(y_mid + last_y),
                    (int) Math.ceil(y_mid + y),
                    pixels,
                    rgb);
            ...
            drawLine(
                    (int) Math.ceil(x_mid + last_x),
                    (int) Math.ceil(x_mid + x),
                    (int) Math.floor(y_mid - last_y),
                    (int) Math.floor(y_mid - y),
                    pixels,
                    rgb);
            drawLine(
                    (int) Math.floor(x_mid - last_x),
        ...
        // drawLine(Math.round(x_mid+last_x), Math.round(x_mid+last_x), Math.round(y_mid-last_y),
        // Math.round(y_mid+last_y), pixels, rgb);
