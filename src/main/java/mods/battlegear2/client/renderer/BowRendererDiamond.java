...
public class BowRendererDiamond implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        item = new ItemStack(BattlegearConfig.modBowDiamond.setFull3D());
        switch (type) {
    ...
    private void renderEquippedBow(ItemStack item, EntityLivingBase entityLivingBase, boolean firstPerson) {
        ...
        if (entityLivingBase instanceof EntityPlayer) {
            ...
            if (quiver != null) {
                arrowStack = ((IArrowContainer2) quiver.getItem())
                        .getStackInSlot(quiver, ((IArrowContainer2) quiver.getItem()).getSelectedSlot(quiver));
            }

            if (drawAmount >= 0) {
                if (arrowStack != null && QuiverArrowRegistry.isKnownArrow(arrowStack)) {
                    icon = ClientProxy.bowDiamondIcons[drawAmount];
                } else {
                    icon = BattlegearConfig.modBowDiamond.getItemIconForUseDuration(drawAmount);
                }
            }
