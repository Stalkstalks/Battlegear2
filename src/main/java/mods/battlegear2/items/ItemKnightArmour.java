...
public class ItemKnightArmour extends ItemArmor implements IHeraldyArmour, ISpecialArmor {
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        ...
        if (armorType == 0) { // Helm
            ...
            for (int i = 0; i < 4; i++) {
                baseIcon[i] = par1IconRegister.registerIcon(
                        "battlegear2:armours/knight-base-" + BattlegearConfig.armourTypes[armorType] + "-" + i);
                postRenderIcon[i] = par1IconRegister.registerIcon(
                        "battlegear2:armours/knight-post-" + BattlegearConfig.armourTypes[armorType] + "-" + i);
            }
        } else {
    ...
    public int getArmorDisplay(EntityPlayer player, ItemStack armor, int slot) {
        if (slot == 1) {
            if (player.getEquipmentInSlot(1) != null
                    && player.getEquipmentInSlot(1).getItem() instanceof ItemKnightArmour) {
                if (player.getEquipmentInSlot(3) != null
                        && player.getEquipmentInSlot(3).getItem() instanceof ItemKnightArmour) {
                    if (player.getEquipmentInSlot(4) != null
                            && player.getEquipmentInSlot(4).getItem() instanceof ItemKnightArmour) {
                        return 9;
                    }
                }
            }
