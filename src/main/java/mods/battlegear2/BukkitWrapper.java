        ...
        try {
            ...
            for (Method m : temp.getMethods()) {
                if (m.getName().equals("callPlayerInteractEvent")) {
                    ...
                    if (i == 3) AirInteract = m;
                    else if (i == 7) BlockInteract = m;
                }
    ...
    public static void callBukkitInteractEvent(PlayerInteractEvent playerInteracted, ItemStack stack) {
        ...
        try {
            ...
            if (playerInteracted.action == PlayerInteractEvent.Action.RIGHT_CLICK_AIR) {
                ...
                if (result != null && ItemUse.invoke(result) == DENY) playerInteracted.useItem = Event.Result.DENY;
            } else {
                result = BlockInteract.invoke(
                        null,
                        playerInteracted.entityPlayer,
                        BLOCK,
                        playerInteracted.x,
                        playerInteracted.y,
                        playerInteracted.z,
                        playerInteracted.face,
                        stack);
