...
public class BaseEnchantment extends Enchantment {
    ...
    public BaseEnchantment(int id, int weight, int limit, int progress, int range) {
        ...
        if (limit > 1) this.max = limit;
        else this.max = 1;
        if (progress > 1) this.enchantCoeff = progress;
        else this.enchantCoeff = 1;
    ...
    public BaseEnchantment(int id, int weight, EnumEnchantmentType type, int coeff, int rng) {
        ...
        this.max = 1;
        if (coeff > 1) this.enchantCoeff = coeff;
        else this.enchantCoeff = 1;
        if (range > 0) this.range = rng;
        else this.range = 0;
    }
    ...
    public Enchantment setName(String name) {
        super.setName(name);
        if (helper.addEnchantment(this)) addToBookList(this);
        return this;
    }
}
