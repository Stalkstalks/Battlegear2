...
public class BattlegearConfig {
    ...
    public static final String[] enchantsName = { "BashWeight", "BashPower", "BashDamage", "ShieldUsage",
            "ShieldRecovery", "BowLoot", "BowCharge" };
    public static ItemWeapon[] dagger = new ItemWeapon[toolTypes.length], warAxe = new ItemWeapon[toolTypes.length],
            mace = new ItemWeapon[toolTypes.length], spear = new ItemWeapon[toolTypes.length];
    ...
    public static void getConfig(Configuration config) {
        ...
        for (int i = 0; i < ItemMBArrow.names.length; i++) {
            skeletonArrowSpawnRate[i] = config
                    .get(category, ItemMBArrow.names[i], i != 1 && i != 5 ? 0.1D : 0.0D, "", 0D, 1D)
                    .setRequiresMcRestart(true).getDouble();
        }
        ...
        if (Arrays.binarySearch(disabledItems, itemNames[11]) < 0) {
            mobBowIron = (ItemBow) new ItemBowIron().setCreativeTab(customTab)
                    .setUnlocalizedName(MODID + itemNames[11] + "." + materials[0])
                    .setTextureName(MODID + itemNames[11] + "." + materials[0]);
            modBowDiamond = (ItemBow) new ItemBowDiamond().setCreativeTab(customTab)
        ...
        if (Arrays.binarySearch(disabledItems, itemNames[1]) < 0) {
            chain = new Item().setUnlocalizedName(MODID + itemNames[1]).setTextureName(MODID + itemNames[1])
                    .setCreativeTab(customTab);
        }

        if (Arrays.binarySearch(disabledItems, itemNames[2]) < 0) {
            quiver = new ItemQuiver().setUnlocalizedName(MODID + itemNames[2])
                    .setTextureName(MODID + "quiver/" + itemNames[2]).setCreativeTab(customTab);
        ...
        if (Arrays.binarySearch(disabledItems, itemNames[9]) < 0) {
            MbArrows = new ItemMBArrow().setUnlocalizedName(MODID + itemNames[9]).setTextureName(MODID + itemNames[9])
                    .setCreativeTab(customTab).setContainerItem(Items.arrow);
                ...
                if (Item.class.isAssignableFrom(f.getType())) {
                    ...
                    if (it != null) {
                        GameRegistry.registerItem(
                                it,
                                it.getUnlocalizedName().replace("item.", "").replace(MODID, "").trim());
                    }
                }
            }
    ...
    public static void registerRecipes() {
        ...
        if (quiver != null) {
            // Quiver recipes :
            if (Arrays.binarySearch(disabledRecipies, itemNames[2]) < 0) GameRegistry
                    .addRecipe(new ShapedOreRecipe(new ItemStack(quiver), "X X", "X X", "XXX", 'X', Items.leather));
        ...
        if (MbArrows != null) {
            for (int i = 0; i < ItemMBArrow.component.length; i++) {
                if (Arrays.binarySearch(disabledRecipies, itemNames[9] + "." + ItemMBArrow.names[i]) < 0) {
                    GameRegistry.addRecipe(
                            new ItemStack(MbArrows, 1, i),
                            "G",
                            "A",
                            'G',
                            ItemMBArrow.component[i],
                            'A',
                            Items.arrow);
                    if (i != 2 && i != 3) { // We can't have those components being duplicated by an "Infinity" bow
                        GameRegistry.addShapelessRecipe(
                                new ItemStack(ItemMBArrow.component[i]),
                                new ItemStack(MbArrows, 1, i));
                    }
                }
            }
        ...
        if (banner != null && Arrays.binarySearch(disabledRecipies, itemNames[10]) < 0) {
            for (int i = 0; i < 7; i++) {
                Object temp = i < 4 ? new ItemStack(Blocks.log, 1, i)
        ...
        /*
         * for(int x = 0; x < 16; x++){ for(int y = 0; y < 16; y++){ ItemStack bannerStack = new ItemStack(bannerItem);
         * ((IHeraldyItem)bannerStack.getItem()).setHeraldryCode(bannerStack,
         * SigilHelper.packSigil(HeraldryPattern.HORIZONTAL_BLOCK, (byte) 0, (byte) 0, new Color(ItemDye.dyeColors[15 -
         * x]), new Color(ItemDye.dyeColors[15 - y]), HeraldryIcon.Blank, HeraldryPositions.SINGLE, Color.WHITE,
         * Color.WHITE) ); GameRegistry.addRecipe(bannerStack, " a "," b ", " S ", 'a', new ItemStack(Block.cloth,0, x),
         * 'b', new ItemStack(Block.cloth,0, y), 'S', Item.stick ); } } GameRegistry.addRecipe(new
         * ItemStack(bannerItem), " W "," W ", " S ",'W',Block.cloth, 'S', Item.stick);
         */
    ...
    /**
     * A symbol for Battlegear mod. Used by creative tab as icon
     *
