...
public class ContainerBattle extends ContainerLocalPlayer {
    ...
    public ContainerBattle(boolean local, EntityPlayer player) {
        ...
        for (int i = 0; i < 4; i++) {
            this.addSlotToContainer(
                    new ArmorSlot(i, inventoryPlayer, inventoryPlayer.getSizeInventory() - 1 - i, 98, 8 + i * 18));
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < 4 || slotIndex > 39) { // we are in weapon/armor slots, so we transfer to normal
            ...
            if (itemStack1.stackSize != itemStack.stackSize) slot.onPickupFromSlot(player, itemStack1);
            else return null;
        }
        return itemStack;
    }
